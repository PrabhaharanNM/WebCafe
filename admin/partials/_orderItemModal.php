                            ...
                            <tbody>
                                <?php
                                    ...
                                    while($myrow = mysqli_fetch_assoc($myresult)){
                                        $coffeeId = $myrow['coffeeId'];
                                        $itemQuantity = $myrow['itemQuantity'];
                                        
                                        $itemsql = "SELECT * FROM `coffee` WHERE coffeeId = $coffeeId";
                                        $itemresult = mysqli_query($conn, $itemsql);
                                        $itemrow = mysqli_fetch_assoc($itemresult);
                                        $coffeeName = $itemrow['coffeeName'];
                                        $coffeePrice = $itemrow['coffeePrice'];
                                        $coffeeDesc = $itemrow['coffeeDesc'];
                                        $coffeeCategorieId = $itemrow['coffeeCategorieId'];

                                        echo '<tr>
                                                <th scope="row">
                                                    <div class="p-2">
                                                    <img src="/OnlinecoffeeDelivery/img/coffee-'.$coffeeId. '.jpg" alt="" width="70" class="img-fluid rounded shadow-sm">
                                                    <div class="ml-3 d-inline-block align-middle">
                                                        <h5 class="mb-0"> '.$coffeeName. '</h5><span class="text-muted font-weight-normal font-italic d-block">Rs. ' .$coffeePrice. '/-</span>
                                                    </div>
                                                    </div>
                                                </th>
